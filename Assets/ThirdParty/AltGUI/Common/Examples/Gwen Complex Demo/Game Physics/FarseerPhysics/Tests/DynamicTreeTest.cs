...
{
    public class DynamicTreeTest : FarseerPhysicsTest
    {
        sealed class Actor
        {
            internal AABB AABB;
        ...
        const int ActorCount = 128;
        Actor[] _actors = new Actor[ActorCount];
        bool _automated;
        double _proxyExtent;
        AABB _queryAABB;
...
#if DEBUG
        RayCastOutput _rayCastOutput;
#endif
        DynamicTree<Actor> _tree = new DynamicTree<Actor>();
        double _worldExtent;
        ...
        private double RayCastCallback(RayCastInput input, int proxyid)
        {
            Actor actor = _tree.GetUserData(proxyid);
